...
func (m *MockNumbered) SplitNote(ctx context.Context, noteLength float64, ts timesig.Time, flag, next musicxml.NoteLength) []NoteLength {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "SplitNote", ctx, noteLength, ts, flag, next)
	ret0, _ := ret[0].([]NoteLength)
	return ret0
}

// SplitNote indicates an expected call of SplitNote.
func (mr *MockNumberedMockRecorder) SplitNote(ctx, noteLength, ts, flag, next interface{}) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "SplitNote", reflect.TypeOf((*MockNumbered)(nil).SplitNote), ctx, noteLength, ts, flag, next)
}
